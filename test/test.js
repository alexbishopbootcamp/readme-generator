...
// Test renderLicenseSection function by comparing the output of the function to a known string
test('renderLicenseSection function should return a license section', () => {
  const testData = "MIT";
  const testString = md.renderLicenseSection(testData);
  const expectedStrig = "## License\n\nThis project uses the [MIT](https://opensource.org/licenses/MIT) license.\n\n";
  expect(testString).toBe(expectedStrig);
});